/* ═══════════════════════════════════════════════════════════
   NARRATOR.JS — Audiobook narrator for Kayfa Nafham al-Islam
...
(function() {
  ...
  function cleanText(text) {
    ...
    if (l === 'fr') {
      clean = clean
        ...
        .replace(/Kayfa Nafham/gi, 'Kaifa Nafham')
    ...
    if (l === 'en') {
      clean = clean
        ...
        .replace(/Kayfa Nafham/gi, 'Kay-fa Naf-ham')
  ...
  function setupMediaSession() {
    ...
    navigator.mediaSession.metadata = new MediaMetadata({
      title: l === 'ar' ? 'كيف نفهم الإسلام' : l === 'fr' ? 'Comment Comprendre l\'Islam' : 'How We Understand Islam',
  ...
  // ═══ SETTINGS ═══
  function onSpeedChange(val) { STATE.speed = parseFloat(val); const label = document.getElementById('narratorSpeedLabel'); if (label) label.textContent = STATE.speed + 'x'; localStorage.setItem('kn-narrator-speed', STATE.speed); }
  function onPitchChange(val) { STATE.pitch = parseFloat(val); const label = document.getElementById('narratorPitchLabel'); if (label) label.textContent = STATE.pitch.toFixed(1); localStorage.setItem('kn-narrator-pitch', STATE.pitch); }
  ...
  function onKaraokeToggle(checked) { STATE.karaokeEnabled = checked; localStorage.setItem('kn-narrator-karaoke', checked); }
  function onAutoScrollToggle(checked) { STATE.autoScroll = checked; localStorage.setItem('kn-narrator-autoscroll', checked); }
  function onDuoToggle(checked) { STATE.duoReading = checked; localStorage.setItem('kn-narrator-duo', checked); }
  ...
  function loadSettings() {
    STATE.speed = parseFloat(localStorage.getItem('kn-narrator-speed')) || 1;
    STATE.pitch = parseFloat(localStorage.getItem('kn-narrator-pitch')) || 1;
    STATE.karaokeEnabled = localStorage.getItem('kn-narrator-karaoke') !== 'false';
    STATE.autoScroll = localStorage.getItem('kn-narrator-autoscroll') !== 'false';
    STATE.duoReading = localStorage.getItem('kn-narrator-duo') === 'true';
  }
